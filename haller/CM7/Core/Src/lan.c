...
void Lan_refresh(void)
{
	...
	if(Lan_state.tcpConnected == 0) //TCP not connected
	{
		...
		{
			if((Lan_state.tcpConnTimeout > 0) && (HAL_GetTick() >= Lan_state.tcpConnTimeout)) //and the timeout occurred
			{
				if(tcp != NULL)
					tcp_abort(tcp); //abort connection and deallocate structure
				lan_tcpInit(); //initialize TCP again
				lan_tcpConnect();
